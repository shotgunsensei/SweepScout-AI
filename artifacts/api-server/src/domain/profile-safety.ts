...
export const profileVaultWarning =
  "Never store SSN, banking information, payment cards, or prize-tax processing details in Play Pack Pilot. Legitimate sweepstakes should not request SSN or banking information until verified winner processing.";
...
export function assertNoForbiddenVaultFields(fields: Iterable<string>) {
  for (const field of fields) {
    ...
    if (forbiddenVaultFieldPattern.test(normalized)) {
      throw new Error("Play Pack Pilot does not store SSN, banking, payment card, or payment information.");
    }
  }
}
...
export function assertNoForbiddenSensitiveText(value: string) {
  if (/\b(?!000|666|9\d{2})\d{3}[- ]?(?!00)\d{2}[- ]?(?!0000)\d{4}\b/.test(value)) {
    throw new Error("Play Pack Pilot does not store SSN values.");
  }

  for (const match of value.matchAll(/\b(?:\d[ -]?){13,19}\b/g)) {
    const digits = match[0].replace(/\D/g, "");
    if (digits.length >= 13 && digits.length <= 19 && luhnValid(digits)) {
      throw new Error("Play Pack Pilot does not store payment card numbers.");
    }
  }
}
