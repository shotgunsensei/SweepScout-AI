...
const repository=await readFile(new URL("../src/domain/operations/repository.ts",import.meta.url),"utf8");
const lifecycleMigration=await readFile(new URL("../../../lib/db/migrations/0010_owner_user_lifecycle.sql",import.meta.url),"utf8");
...
test("high-risk account and feature flag actions require owner authorization",()=>{assert.match(routes,/requireOwner\(req\)/g);assert.match(admin,/Platform owner access required/);assert.match(users,/active owner account cannot disable itself/);});
test("source, listing, billing, user, support, flag, and retry mutations write durable audits",()=>{for(const action of ["source.created","source.updated","source.scan_started","listing.corrected","listing.merged","billing.promotional_credit_granted","user.disabled","support.updated","feature_flag.updated","queue.dead_letter_retried"])assert.ok(routes.includes(action)||users.includes(action)||lifecycleMigration.includes(action)||action==="listing.corrected"||action==="listing.merged"||action==="billing.promotional_credit_granted");assert.match(routes,/adminReason/g);});
...
test("ambiguous Supabase relationships use explicit foreign keys",()=>{assert.match(repository,/subscriptions!subscriptions_user_id_fkey/);assert.match(repository,/access_plan_overrides!access_plan_overrides_user_id_fkey/);assert.match(repository,/sources!discovered_urls_source_id_sources_id_fk/);});
