...
test("all metered AI routes use the central consume, refund, and effective-plan service",()=>{assert.ok((routes.match(/withPilotCredits\(/g)??[]).length>=3);assert.match(credits,/repository\.consume/);assert.match(credits,/repository\.refund/);assert.match(credits,/resolveEffectivePlan\(repository,input\.userId\)/);});
