...
test("every opportunity card exposes sponsor, source, verification, and platform boundary", () => { for (const term of ["item.sponsor", "item.sources", "item.lastVerifiedAt", "not the sponsor or promotion administrator", 'referrerPolicy="no-referrer"']) assert.match(card, new RegExp(term.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"))); });
