...
const [page,users]=await Promise.all([
  ...
  readFile(new URL("../src/components/admin/users.tsx",import.meta.url),"utf8")
]);
...
test("source, listing, user, billing, support, dead-letter, flags, and audits are operable",()=>{for(const text of ["Source administration","Create disabled source","Run scan","Listing review","AI field evidence, rules versions, and change history","Users and Access","Credit Ledger","Adjust Balance","Disable Access","Support requests","Retry dead letter","Feature flags","Immutable audit log"])assert.ok(`${page}\n${users}`.includes(text),text);});
