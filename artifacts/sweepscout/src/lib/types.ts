...
export type FlightDeckData = { generatedAt:string;opportunities:RadarOpportunity[];notifications:AlertNotification[];stats:{opportunityMatches:number;eligibleMatches:number;endingSoon:number;riskFlags:number;saved:number;entriesToday:number;dueEntries:number;unreadAlerts:number} };
...
export type AdminUser = {
  ...
  email: string;
  display_name: string;
  platform_role: "user" | "admin" | "owner";
  account_disabled_at: string | null;
  account_removed_at: string | null;
  created_at: string;
  updated_at: string;
  subscriptions?: Array<{ plan_key: string; status: string; current_period_end: string | null }>;
  access_plan_overrides?: Array<{ plan_key: string; active: boolean; reason: string; set_by: string; updated_at: string }>;
};
export type AdminOperations = {
  ...
  users:AdminUser[];creditLedger:Array<Record<string,any>>;sources:Array<Record<string,any>>;sourceFailureRates:Array<{sourceId:string;sampleSize:number;failures:number;rate:number}>;scanJobs:Array<Record<string,any>>;failedUrls:Array<Record<string,any>>;listings:Array<Record<string,any>>;reviewFlags:Array<Record<string,any>>;billingFailures:Array<Record<string,any>>;applicationErrors:Array<Record<string,any>>;supportRequests:Array<Record<string,any>>;auditLogs:Array<Record<string,any>>;featureFlags:Array<Record<string,any>>;deadLetters:Array<Record<string,any>>;
};
