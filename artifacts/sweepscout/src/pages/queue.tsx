    ...
    <AppShell>
      ...
      {data ? (
        <div className="grid gap-4">
          {tasks.length ? (
            tasks.map((task) => <QueueTaskCard key={task.id} task={task} />)
          ) : (
            <EmptyState
        ...
        <div className="flex flex-col gap-2 xl:min-w-52">
          <form
            onSubmit={(event) => {
              ...
              approve.mutate(formToObject(event.currentTarget));
            }}
          >
            <input type="hidden" name="taskId" value={task.id} />
            <SubmitButton disabled={approve.isPending}>
          ...
          <form
            ...
            onSubmit={(event) => {
              event.preventDefault();
              record.mutate(formToObject(event.currentTarget));
            }}
          >
            ...
            <div className="grid gap-2">
              ...
              <SubmitButton tone="secondary" disabled={record.isPending}>Log Attempt</SubmitButton>
            </div>
          </form>
        </div>
      </div>
