...
test("security-definer owner RPCs are revoked from public and granted only to service role", () => {
  assert.match(sql, /REVOKE ALL ON FUNCTION admin_apply_user_lifecycle\(uuid,uuid,text,text,text,text\) FROM PUBLIC/);
  assert.match(sql, /REVOKE ALL ON FUNCTION admin_set_access_plan_override\(uuid,uuid,text,text,text\) FROM PUBLIC/);
