...
import { getEntryTrackingData, type EntryQueueItem, type ReminderDay } from "@/lib/services/entry-tracking";
import type { EntryLog, Sweepstake } from "@/lib/types";

export const dynamic = "force-dynamic";

export default async function DashboardEntriesPage() {
  const data = await getEntryTrackingData();
...
function EntryActions({ sweepstake, submitted = false }: { sweepstake: Sweepstake; submitted?: boolean }) {
  return (
    <div className="flex flex-wrap items-start gap-2 xl:justify-end">
      {submitted ? (
        <form action={markEntryStatusAction} className="rounded-md border border-line bg-panel p-3">
...
function StatusButton(props: {
  ...
  return (
    <form action={markEntryStatusAction} className="rounded-md border border-line bg-panel p-3">
      ...
      <SubmitButton tone={props.status === "suspicious" || props.status === "expired" ? "danger" : "secondary"}>
        <span className="inline-flex items-center gap-2">
