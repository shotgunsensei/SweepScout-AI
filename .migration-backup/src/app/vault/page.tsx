...
import { profileVaultWarning } from "@/lib/profile-safety";
...
export default async function VaultPage() {
  ...
  return (
    <AppShell>
      <PageHeader title="Secure Profile Vault" kicker={profile.email}>
        <Badge tone={profile.consentToPrefill ? "ok" : "warn"}>
          {profile.consentToPrefill ? "Prefill consent enabled" : "Prefill consent off"}
        </Badge>
            ...
            <section className="grid gap-3">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <Field label="Max daily entries">
                  <TextInput
                    name="maxDailyEntries"
                    type="number"
                    min={1}
                    max={100}
                    defaultValue={profile.preferences.maxDailyEntries}
                  />
                </Field>
              </div>
              <div className="flex flex-wrap gap-4">
                <Checkbox
                  name="avoidPurchaseRequired"
                  label="Avoid purchase-required promotions"
                  defaultChecked={profile.preferences.avoidPurchaseRequired}
                />
                <Checkbox
                  name="allowSocialActions"
                  label="Allow sweepstakes that require social actions"
                  defaultChecked={profile.preferences.allowSocialActions}
                />
              </div>
              <div className="rounded-md border border-line bg-panel-strong p-4">
                <Checkbox
                  name="consentToPrefill"
                  label="Enable form prefill from this vault"
                  defaultChecked={profile.consentToPrefill}
                />
                ...
                {!profile.consentToPrefill ? (
                  <div className="mt-4">
                    <Checkbox
                      name="prefillConfirmation"
                      label="I confirm I want to enable prefill and will review every entry manually"
                    />
                  </div>
                ) : (
